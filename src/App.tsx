...
function App() {
  ...
  useEffect(() => {
    const titleElement = document.querySelector("div#item-area div.item-info-area > h1");
    if (!titleElement) {
      throw new Error("div#item-area div.item-info-area > h1 Element is not found");
    }
  ...
  return (
    <>
      {buttons.map((button) => (
        <a className="btn" href={button.url} target="_blank" rel="noreferrer" key={button.text}>
